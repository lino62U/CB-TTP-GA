...
def load_input(path: str) -> Dict[str, Any]:
    ...
    if isinstance(profs_raw, dict):
        ...
        for info in profs_raw:
            name = info.get('nombre') or info.get('id') or info['correo']
            profs[name] = info
    ...
    for c in data.get('cursos', []):
        ...
        hrs = c.get('horas') or c.get('horas', {})
...
def evaluate(ind: Dict[str, List[Tuple[Period, AulaID]]], data: Dict[str, Any]) -> float:
    ...
    # default fallbacks
    w_H = {
        'H1': hard_weights.get('no_solapamiento_profesor', M),
...
def main():
    ...
    pretty_print_solution(best, data, out_path="best_schedule.txt")
    print("Se guardó best_schedule.txt con la solución (curso -> periodos y aulas).")
